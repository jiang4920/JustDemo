...
public class SQLiteUtil extends SQLiteOpenHelper {
	...
	private String SQL_CREATE_TABLE_USER = "create table if not exists "
			+ TABLE_USER
			+ " (uid INTEGER primary key not null, nickname TEXT not null, email TEXT not null, expiretime INTEGER, loginTime INTEGER not null, keepLogin INTEGER default(0))";
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		if(newVersion > oldVersion){
			db.execSQL(SQL_DELETE_TABLE+"USER");
			db.execSQL(SQL_CREATE_TABLE_USER);
		}
	}
}
